...
```
완료: 28/31 (C0 `4e28936` · C1-1~5 · C2-1~5 · C3-1~6 · C4-1~6 · C5-1~3 · C6-3 조건 미충족 종료 · C6-5)  |  **보류: C6-1·C6-2·C6-6 — 사용자가 글로벌 설정을 별도 세션에서 작업**(2026-09-23 결정)  |  제외: C6-4  |  커밋·PR 완료 (아래 Results)(2026-09-23 사용자 결정)  |  제외: 1 (C6-4)   (결정 대기: 0)   ※ 하위 항목 기준: C0 · C1-1~5 · C2-1~5 · C3-1~6 · C4-1~6 · C5-1~3 · C6-1~3·5·6
```
...
> ~~**잔여**: 프론트 레포 신뢰 수락(F14) 후 `../bun` 실제 Read 권한 확인.~~ ✅ 해소(F14 — 신뢰 수락 후 headless 세션에서 `../bun` Read 성공)
>
...
- **회귀 수정 (2026-09-23)** — 사용자 질문 "날짜 설정을 바꿔서 날짜 작업 때 참조를 안 하게 된 건 아닌가"로 점검. d742649(`paths`: `src`·`test`만) + ec1cd64(라우팅 행 삭제)가 겹쳐 **`migrations/`(레포 루트) 작업 때 code-patterns §12·DB 스키마 체크리스트가 로드되지 않던 회귀** 발견 → `9562eeb`에서 `paths`에 `migrations/**/*.ts`·`migration-datasource.ts` 추가. 핵심 날짜 금지(CLAUDE.md Date/Time 절)는 항상 로드라 영향 없었다. 실측: 새 세션에서 migrations 파일 Read → §12 로드 / 프론트 세션 훅도 주입.
- **PR #239 (bun)** — 라우팅 표 정리 후속: [#239](https://github.com/kon6443/bun/pull/239).
- **🔴 main 배포 verify 게이트 첫 실행 실패 → 복구 (2026-09-23)** — next-bun #316 → develop → main(#317) 머지 후 run `35832641052`가 **verify › Typecheck** 실패, deploy는 skip(프로덕션 무영향 — 게이트가 의도대로 막음). 원인: `src/app/utils/taskUtils.test.ts` mock 데이터가 `Task` 타입의 `teamId`·`completedAt`을 누락한 **기존 타입 드리프트**(TS2739 18건, vitest는 타입 미검사라 통과해 왔다). **내 검증 실수**: C5-1 기준선을 이 파일의 미커밋 수정(작업 이전부터 존재)이 섞인 작업 트리에서 재서 "typecheck 통과"라 보고했다. 복구: 그 수정만 커밋(`e485cad`, 사용자 승인) → [next-bun#318](https://github.com/kon6443/next-bun/pull/318). `origin/main` 워크트리 + 이 파일만으로 재현: tsc 0 · vitest 31/31 · lint 에러 0. 교훈 `docs/lessons.md` 2026-09-23. 백엔드 Actions 실패는 2026-08-12 #235 1건뿐이고 이후 #237 성공 — 현재 문제 없음.
