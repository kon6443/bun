...
| 문서 | 담당 |
|---|---|
| **[코드 패턴](.claude/rules/code-patterns.md)** | 계층·DB·트랜잭션·에러·인증·응답·테스트 규약 SSOT (실측 카운트 병기) — `src`·`test`·`migrations`의 `.ts`를 읽으면 자동 로드 |
...
2. `CLAUDE.md`의 문서 라우팅 표에서 작업 유형에 맞는 문서 확인 (코드 규약·형제 레포 규약은 자동 로드)
